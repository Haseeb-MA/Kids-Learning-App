...
export default function ContentManagerDashboard() {
  ...
  useEffect(() => {
    document.title = 'Content manager · BrightMinds'
    checkAccessAndLoad()
  }, [])
  ...
  const checkAccessAndLoad = async () => {
    ...
    if (profile?.role !== 'parent' || !profile?.is_content_manager) {
      router.push('/login')
      return
    }
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: '14px', color: '#888780' }}>
              Manage subjects, lessons and quizzes
            </p>
          </div>
          <button
                  ...
                  <tr key={subject.id} style={{
                    ...
                    <td style={tdStyle}>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button
                          onClick={() => router.push(`/admin/content/subject/${subject.id}?from=content-manager`)}
                          style={{
                            padding: '5px 10px',
                            background: '#EEEDFE',
